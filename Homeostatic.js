...
homeostatic.configure = function () {
    ...
    for (let a = 0; a < real.length; a++) {
        ...
        realData.push({
            ...
            isDisplayFixedByInlineStyle: real[a].style.display,
            ...
            children: [],
            currentWidth: window.getComputedStyle(real[a]).getPropertyValue("width"),
            display: window.getComputedStyle(real[a]).getPropertyValue("display"),
            childOnLeave: [],
        });
        document.documentElement.style.setProperty("--" + real[a].getAttribute("data-h-parent")?.split(",")[0] + "-width", window.getComputedStyle(real[a]).getPropertyValue("width"))
    }
    ...
    for (d = 0; d < foster.length; d++) {
        ...
        fosterData.push({
            ...
            isDisplayFixedByInlineStyle: foster[d].style.display,
        ...
        document.documentElement.style.setProperty("--" + foster[d].getAttribute("data-h-parent")?.split(",")[0] + "-width", window.getComputedStyle(foster[d]).getPropertyValue("width"));
    }
    ...
    for (c = 0; c < homeostatic.realParents.length; c++) {
        ...
        for (let b = 0; b < child.length; b++) {
            ...
            document.documentElement.style.setProperty("--" + homeostatic.realParents[c].name + "-" + b + "-width", window.getComputedStyle(child[b]).getPropertyValue("width"))
        }
    ...
    for (i = 0; i < homeostatic.realParents.length; i++) {
        ...
        if (
            ...
            if (homeostatic.realParents[i].percentage != null) {
                ...
                document.documentElement.style.setProperty("--" + homeostatic.realParents[i].name + "-width", String(percent) + "px")
            }
                    ...
                    case "manage":
                        if (children[k].percentage != null) {
                            if (children[k].percentage == "inRatio") {
                                ...
                                document.documentElement.style.setProperty("--" + children[k].name + "-width", children[k].domElement.style.width);
                            } else {
                                children[k].domElement.style.width =
                                    String(
                                        (Number(
            ...
            if (homeostatic.realParents[i].isWidthFixedByInlineStyle == "") {
                homeostatic.realParents[i].domElement.style.width = "";
                document.documentElement.style.setProperty("--" + homeostatic.realParents[i].name + "-width",window.getComputedStyle(homeostatic.realParents[i].domElement).getPropertyValue("width"));
            } else {
                homeostatic.realParents[i].domElement.style.width =
                    homeostatic.realParents[i].width;
                    document.documentElement.style.setProperty("--" + homeostatic.realParents[i].name + "-width", homeostatic.realParents[i].width)
            }
                    ...
                    switch (children[n].type) {
                        case "takeBirth":
                            ...
                            if (children[n].isWidthFixedByInlineStyle == "") {
                                ...
                                document.documentElement.style.setProperty("--" + children[n].name + "-width", windeow.getComputedStyle(children[k].domElement).getPropertyValue("width"));
                            } else {
                                children[n].domElement.style.width = children[n].width;
                                document.documentElement.style.setProperty("--" + children[n].name + "-width", children[k].domElement.style.width);
                            }
    ...
    for (w = 0; w < foster.length; w++) {
        ...
        if (
            ...
            if (
                ...
                document.documentElement.style.setProperty("--" + foster[w].name + "-width", String(percent) + "px")
                ...
                for (z = 0; z < children.length; z++) {
                    ...
                    if (children[z].percentage != null) {
                        if (children[z].percentage == "inRatio") {
                            ...
                            document.documentElement.style.setProperty("--" + children[z].name + "-" + "-widthWhenGotOut", children[k].currentDomElement.style.width);
                        } else {
                            children[z].currentDomElement.style.width =
                                String(
                                    (Number(foster[w].currentWidth.split("px").join("")) *
                            ...
                            document.documentElement.style.setProperty("--" + children[k].name + "-widthWhenGotOut", children[k].domElement.style.width);
                            children[z].isManaged = true;
                        }
                    }
                }
            } else {
                if (foster[w].isWidthFixedByInlineStyle == "") {
                    foster[w].domElement.style.width = "";
                    document.documentElement.style.setProperty("--" + foster[w].name + "-width", window.getComputedStyle(foster[w].domElement).getPropertyValue("width"))
                } else {
                    foster[w].domElement.style.width = foster[w].width;
                    document.documentElement.style.setProperty("--" + foster[w].name + "-width", foster[w].domElement.style.property.width)
                }
...
function isWidthOrDisplayChanged(node, styleOldValue) {
    ...
    if (widthChange != false && displayChange != false) {
        if (domTask.find((item) => {
            return ((item.value == widthChange[1]) && (item.attribute == widthChange[0]) && (item.node == node))
        }) != undefined) {
...
function callback(list, observer) {
    ...
    homeostatic.realParents.forEach((item, index) => {
        if (item.isDisplayFixedByInlineStyle == "") {
            item.domElement.style.display = "";
        } else {
        ...
        item.children.forEach((value) => {
            if (value.isDisplayFixedByInlineStyle == "") {
                item.domElement.style.display = "";
            } else {
                value.domElement.style.display = item.display;
            }
